...
export function executeClearDisplay(service: LCDService): void {
  // Uses existing sendCommand usecase to ensure log/event parity
  service.sendCommand(0x01);
}

export function executeSetAddress(service: LCDService, address: number): void {
  // Pattern 0b1xxxxxxx
  const command = 0x80 | (address & 0x7F);
  service.sendCommand(command);
}

export function executeReset(service: LCDService): void {
