...
function toChar(byte) {
    if (byte >= 0x20 && byte <= 0x7E)
        return String.fromCharCode(byte);
    if (byte >= 0x00 && byte <= 0x07)
        return '■'; // CGRAM custom characters
    return '.';
}
function dumpDDRAM() {
...
lcdService.eventBus.subscribe(event => {
    ...
    if (event.type === 'STATE_UPDATED') {
        ...
        const msg = event.type === 'ERROR' ? event.message : event.log;
        logs.push(typeof msg === 'string' ? msg : JSON.stringify(msg));
        if (logs.length > 50)
            logs.shift(); // Keep more in memory, but render limits slice
        shouldRender = true;
    }
    if (shouldRender) {
        render();
    ...
    try {
        switch (cmd) {
            ...
            case 'pulse':
                ...
                break;
            case 'state':
                logs.push('LCD State: ' + JSON.stringify(lcdService.getState(), (key, val) => key === 'ddram' || key === 'cgram' ? `Uint8Array(${val.length})` : val, 2));
                break;
            case 'ddram':
                dumpDDRAM();
                break;
