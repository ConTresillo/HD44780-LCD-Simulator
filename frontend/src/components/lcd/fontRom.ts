/**
 * fontRom.ts — Standard HD44780 Font ROM (A00).
 * Provides 5x8 bitmaps for standard characters.
 */

const FONT_MAP: Record<number, number[]> = {
  ...
  0x4A: [0x01, 0x01, 0x01, 0x01, 0x01, 0x11, 0x0E, 0x00], // 'J'
  ...
  0x59: [0x11, 0x11, 0x11, 0x0A, 0x04, 0x04, 0x04, 0x00], // 'Y'
  ...
  0x66: [0x06, 0x09, 0x1C, 0x08, 0x08, 0x08, 0x08, 0x00], // 'f'
  ...
  0x70: [0x00, 0x00, 0x1E, 0x11, 0x1E, 0x10, 0x10, 0x10], // 'p'
  0x71: [0x00, 0x00, 0x0D, 0x13, 0x0F, 0x01, 0x01, 0x01], // 'q'
  ...
  0x79: [0x00, 0x00, 0x11, 0x11, 0x0F, 0x01, 0x11, 0x0E], // 'y'
  ...
  0x2C: [0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x06, 0x04], // ','
  0x3A: [0x00, 0x06, 0x06, 0x00, 0x06, 0x06, 0x00, 0x00], // ':'
  0x3B: [0x00, 0x06, 0x06, 0x00, 0x06, 0x06, 0x04, 0x00], // ';'
  // Box fallback for unknown characters
  0x3F: [0x0E, 0x11, 0x01, 0x02, 0x04, 0x00, 0x04, 0x00], // '?'
};

export function charToBitmap(code: number): number[][] {
  // Codes 0-7 are CGRAM, but this function handles the Font ROM fallback.
  // We return a blank space for 0x00 to avoid conflicts if glyphs are missing,
  // but LcdDisplay handles the actual priority.
  const rows = FONT_MAP[code] || FONT_MAP[0x3F]; // Fallback to '?'
